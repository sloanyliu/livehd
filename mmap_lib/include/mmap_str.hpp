...
mmap_lib::map<std::string_view, uint32_t> &map_ref() { return map_id == 1 ? map1 : map_id == 2 ? map2 : map_id == 3 ? map3 : map0; }

mmap_lib::vector<int> &vec_ref() { return map_id == 1 ? vec1 : map_id == 2 ? vec2 : map_id == 3 ? vec3 : vec0; }

mmap_lib::map<std::string_view, uint32_t> &map_cref() const {
  return map_id == 1 ? map1 : map_id == 2 ? map2 : map_id == 3 ? map3 : map0;
}

mmap_lib::vector<int> &vec_cref() const { return map_id == 1 ? vec1 : map_id == 2 ? vec2 : map_id == 3 ? vec3 : vec0; }

int get_map_id() const { return map_id; }

static void clear_map() { map_id == 1 ? map1.clear() : map_id == 2 ? map2.clear() : map_id == 3 ? map3.clear() : map0.clear(); }

static void clear_vec() { map_id == 1 ? vec1.clear() : map_id == 2 ? vec2.clear() : map_id == 3 ? vec3.clear() : vec0.clear(); }
